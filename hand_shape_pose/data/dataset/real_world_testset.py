...
class RealWorldTestSetVideo(torch.utils.data.Dataset):
    def __init__(self, root, param_file, ann_file, **kwargs):
        self.vid_name = kwargs['vid_name']
        self.cap = cv2.VideoCapture(self.vid_name)
        self.data_path = root

        # TODO: simplify and discard what isn't needed
        mat_params = sio.loadmat(param_file)
        self.image_paths = mat_params["image_path"]
